...
CASES = [
    ('safe', 'BEGIN {system("printf SAFE_PROBE")}', '', 'safe'),
    ('csv', '{print NF,$2}', 'a,"b,c",d\n', 'csv'),
    ...
    with tempfile.TemporaryDirectory(prefix='rawk-audit-') as work:
        for case in CASES:
            ...
            for label, binary in [('c', ROOT/'c_awk/a.out'), ('rust', ROOT/'rawk/target/release/rawk')]:
                try:
                    options = (['-safe'] if label == 'c' else ['--safe']) if mode == ['safe'] else (['--csv'] if mode == ['csv'] else [])
                    out = subprocess.run([str(binary), *options, *args], input=stdin, capture_output=True,
                                         cwd=work, env=env, timeout=3)
                    row[label] = {'code': out.returncode, 'stdout_hex': out.stdout.hex(), 'stderr_hex': out.stderr.hex(), 'stdout': out.stdout.decode(errors='backslashreplace'),
                                  'stderr': out.stderr.decode(errors='backslashreplace')}
                except subprocess.TimeoutExpired:
                    row[label] = {'timeout': True}
            row['same_stdout_and_status'] = all(row['c'].get(k) == row['rust'].get(k) for k in ('code','stdout_hex','timeout'))
            results.append(row)
    output = ROOT / 'rawk/diary/audit-regression-results.json'
